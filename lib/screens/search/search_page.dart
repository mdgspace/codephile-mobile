...
import 'package:codephile/screens/search/filter_search.dart';
...
class _SearchPageState extends State<SearchPage> {
  ...
  List<CodephileUser> result = <CodephileUser>[];
  List<CodephileUser> filteredResult = <CodephileUser>[];
  int statusCode = 0;
                              ...
                              IconButton(
                                ...
                                onPressed: () {
                                  showModalBottomSheet<String?>(
                                    ...
                                    builder: (context) {
                                      return FilterSearch(
                                        selectedInstitute: filterInstitute,
                                      );
                                    },
                                  ).then((value) {
                                    if (value == null) {
  ...
  void filterResult() {
    if (filterInstitute == 'All') {
      ...
      setState(() {
        isResultNull = filteredResult.isEmpty;
    ...
    for (var user in result) {
      if (user.institute == filterInstitute) {
